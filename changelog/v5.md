...
- **Component directory structure**: New `components/{type}s/{name}/` layout
  - `components/servers/{name}/` for server components
  - `components/webapps/{name}/` for webapp components
  - `components/contracts/{name}/` for contract components
  - `components/databases/{name}/` for database components
  - `components/helm-charts/{name}/` for helm components
  - `components/testing/{name}/` for testing components
  - `components/config/` remains flat (mandatory singleton)

- **project.ts**: Added `pluralizeType()` function for consistent directory naming
  - Custom mappings: `helm` → `helm-charts`, `testing` → `testing`
  - Default: adds 's' suffix (e.g., `server` → `servers`)

- **sync.ts**: Updated `getComponentDir()` to match new structure

- **Documentation**: Updated all references to new directory structure
  - Skills: scaffolding, contract-scaffolding, contract-standards, config-standards, frontend-scaffolding, frontend-standards
  - Agents: api-designer, backend-dev, ci-dev
  - Commands: sdd-config
  - Templates: project/CLAUDE.md

### Rationale

The new directory structure groups components by type, making it easier to navigate projects with multiple components of the same type. Component names no longer require a type prefix since the type is encoded in the parent directory.
...
- **check-tools skill**: Reusable tool verification for environment setup
  - Required tools: node, npm, git, docker (fails if missing)
  - Optional tools: jq, kubectl, helm (warns if missing)
  - Used by sdd-init for environment verification

- **permissions command**: New `/sdd-run permissions configure` command
  - Automatically configures SDD permissions in `.claude/settings.local.json`
  - Backs up existing settings before modifying
  - Merges SDD permissions with existing user permissions

- **Read permission**: Added `Read(~/.claude/plugins/sdd/**)` to recommended-permissions.json
  - Enables plugin file access for check-tools skill
  - Documented in PERMISSIONS.md

- **projectFileDoesNotExist test helper**: New assertion for verifying file absence

### Changed

- **sdd-init command**: Complete redesign for minimal, change-driven workflow
  - No longer takes arguments - project name derived from current directory
  - Phase 0: Detect project name from directory
  - Phase 1: Environment verification (tools, plugin, permissions)
  - Phase 2: Component selection (defer scaffolding for later)
  - Phase 3: Create minimal structure (only config component scaffolded)
  - Phase 4: Git initialization
  - Phase 5: Display completion summary
  - Non-destructive: never overwrites existing files
  - Removes product-discovery and domain-population phases

- **sdd-new-change command**: On-demand scaffolding and mandatory discovery
  - Step 3: Mandatory discovery skills (product-discovery, component-recommendation, domain-population)
  - Step 4: On-demand scaffolding for unscaffolded components
  - Discovery skills run for EVERY change to ensure context is populated
  - Components scaffolded only when first needed

- **project-scaffolding skill**: Added minimal vs full mode support
  - `mode: "minimal"` - Only scaffolds config component
  - `mode: "full"` - Scaffolds all provided components (existing behavior)
  - Minimal mode used by sdd-init, full mode used by on-demand scaffolding

- **sdd-init.test.ts**: Rewritten for minimal init workflow
  - Verifies only config component is scaffolded
  - Asserts changes/, specs/domain/, other components do NOT exist
  - Updated prompts for automated test mode

### Rationale

The previous sdd-init did too much upfront - product discovery, full component scaffolding, and spec population before the user had written any code. This release makes init minimal and moves scaffolding to when components are first needed via sdd-new-change. The discovery skills are now mandatory for every change to ensure domain context stays current.
...
- **product-discovery skill**: External spec mode (`mode: "external-spec"`) for non-interactive extraction
  - Extracts domain entities, user personas, and core workflows from spec content
  - Populates domain specs (glossary, definitions, use-cases) automatically

- **spec-decomposition skill**: Hierarchical decomposition mode (`mode: "hierarchical"`)
  - Two-level analysis: H1 → epics, H2/H3 → features
  - Topological sort determines implementation order
  - Output includes numbered prefixes based on dependency order

- **external-spec-integration skill**: Numbered hierarchical structure for large specs
  - Epics named `01-epic-name`, `02-epic-name`, etc.
  - Features named `01-feature`, `02-feature` within epics
  - Numbers indicate implementation order (dependencies resolved via topological sort)

### Changed

- **INDEX.md relocation**: Moved from `specs/INDEX.md` to `changes/INDEX.md`
  - INDEX.md tracks changes, so it belongs in `changes/` not `specs/`
  - Updated: project-scaffolding, change-creation, spec-index skills
  - Updated: generate-index CLI command, project scaffolding templates
  - Backwards-compatible: CLI still accepts `--specs-dir` as legacy option

- **sdd-new-change command**: Product discovery now runs before decomposition for external specs
  - New step 3 invokes product-discovery with `mode: "external-spec"`
  - Discovery results passed to external-spec-integration for better decomposition

- **Workflow tests**: Updated to use `changes/INDEX.md` path
  - sdd-init.test.ts, sdd-new-change.test.ts, sdd-new-change-external-spec.test.ts
  - Added verification for numbered epic directories in external spec test

### Rationale

External specs were creating flat lists instead of hierarchical epic structures. This release fixes the regression by:
1. Running product discovery to understand domain before decomposition
2. Using hierarchical decomposition (H1→epics, H2/H3→features)
3. Numbering directories by implementation order
4. Moving INDEX.md to `changes/` where it semantically belongs
...
- **planning skill**: Plans now focus on WHAT (outcomes), not HOW (implementation tasks)
  - Updated SPEC vs PLAN table with "Does NOT Contain" column
  - Added "Plan Content Guidelines" section with acceptable vs inappropriate content
  - Added spec self-sufficiency principle statement
  - Replaced "Tasks" checklists with "Outcome" descriptions in phase templates

- **tasks skill**: Updated plan template to focus on changes, not implementation
  - New sections: Changes, Dependencies, Tests
  - Added "Plan Content Guidelines" table
  - Required extensive test lists in all plans

- **docs/workflows.md**: Fixed PLAN.md description from "How to build it" to "Execution phases and agent coordination"

### Rationale

Plans were duplicating implementation details that belong in specs. This release clarifies the boundary: SPEC.md contains the technical HOW, PLAN.md coordinates execution (WHAT phases, WHO implements, WHEN it's done). This reduces duplication, makes plans more stable, and ensures specs are the single source of implementation truth.
...
- **Local environment management**: New `env` CLI namespace for managing local Kubernetes development environments
  - `env create` - Create local k8s cluster + install observability stack
  - `env destroy` - Delete cluster entirely
  - `env start` / `env stop` / `env restart` - Lifecycle management with state preservation
  - `env status` - Show cluster and workload status
  - `env deploy` - Orchestrate: databases → migrations → helm charts
  ...
  - `env infra` - Install/reinstall Victoria Metrics + Victoria Logs

- **Cluster provider abstraction**: Support for multiple local k8s providers
...
- **local-env skill**: Complete documentation for local environment workflow
  - Prerequisites, quick start, architecture, commands reference
  - Hybrid development pattern (run some services locally)
  - Troubleshooting guide

### Enhanced

- **devops agent**: Added local environment workflow section with `/sdd-run env` commands
- **helm-standards skill**: Added `secretKeyRef` documentation for secret references
- **helm-scaffolding skill**: Added `{{CHART_DESCRIPTION}}` and `SDD_CONFIG_PATH` documentation

### Fixed

- **ESM module resolution**: Fixed tsc-alias to add .js extensions for Node.js ESM compatibility
- **Test file bug**: Removed duplicate `execSync` declaration in sdd-new-change-external-spec.test.ts

### Rationale

Local environment management is critical for developer productivity. This release provides a complete workflow for spinning up local Kubernetes clusters with observability infrastructure, deploying applications, and managing the development lifecycle.
...
- **plugin.json**: Remove duplicate hooks reference causing "Duplicate hooks file detected" error
  - `hooks/hooks.json` is auto-loaded by Claude Code, manifest should not also reference it
...
- **Component settings system**: First-class settings that drive scaffolding, config, and Helm charts
  - TypeScript types in `plugin/system/src/types/settings.ts`
  - JSON Schema validation in `plugin/system/src/settings/schema.ts`
  - Default settings per component type in `plugin/system/src/settings/defaults.ts`
  - Cross-reference validation (databases, contracts, helm references)
  - Settings sync system for incremental updates

- **Helm chart templates**: Complete settings-driven templates
  - Server templates: deployment, service, ingress, servicemonitor, cronjob, hybrid modes
  - Webapp templates: nginx deployment with config injection via init container
  - Umbrella chart template for multi-component projects
  - Located in `plugin/skills/helm-scaffolding/templates-{server,webapp,umbrella}/`

- **/sdd-settings command**: View and modify component settings
  - Interactive preview of changes before applying
  - Auto-commit protection for uncommitted work
  - Automatic sync of affected artifacts

- **105 unit tests** for the settings system
  - Types, defaults, validation, sync, and schema tests
  - Located in `tests/src/tests/unit/settings/`

### Enhanced

- **backend-scaffolding skill**: Settings-driven conditional scaffolding
  - DAL layer per database, HTTP routes per contract, API clients for consumed contracts

- **config-scaffolding skill**: Settings-driven config section generation
  - Database sections, queue config for workers, API client sections

- **helm-scaffolding skill**: Comprehensive settings-driven chart generation docs

- **helm-standards skill**: Expanded with chart-per-server pattern, observability integration

- **project-settings skill**: Full settings schema documentation and lifecycle

- **component-recommendation skill**: Now includes settings in recommendations

- **devops agent**: Added settings-driven infrastructure and observability integration

### Rationale

Component settings enable structural decisions (databases, contracts, server types) to automatically drive what gets scaffolded and how. This eliminates manual coordination between sdd-settings.yaml, config files, and Helm charts.

Consolidates tasks #44, #46 (Helm chart template), #53 (helm-standards skill), #57 (/sdd-settings command).
